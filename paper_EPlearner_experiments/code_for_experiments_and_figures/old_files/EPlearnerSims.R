...
# Generate data
base_path <- "~/hte3/paper_EPlearner_experiments"

if (sim_type == "CATEhigh") {
  source(file.path(base_path, "SimulationRCode", "generate_data_CATE_HighDim.R"))
  nsims <- 500
  sim.fun <- sim.CATEHighDim

} else if (sim_type == "CATElow") {
  source(file.path(base_path, "SimulationRCode", "generate_data_CATE.R"))
  sim.fun <- sim.CATE
}
