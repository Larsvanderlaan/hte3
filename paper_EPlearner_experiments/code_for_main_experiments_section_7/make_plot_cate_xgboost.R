...
FIGURES_DIR <- file.path(BASE_PATH, "experiment_results")  # (you set this; keeping consistent)
PLOTS_DIR   <- file.path(FIGURES_DIR, "plots")


dir.create(PLOTS_DIR,   recursive = TRUE, showWarnings = FALSE)

# Canonical helpers (no scattered strings)
sim_file <- function(hard, pos, n) {
  file.path(RESULTS_DIR, "mainSimResults2", "mainSimResults2",
            paste0("simsCATE", hard, pos, "n", n, "_xgboost"))
}
    ...
    try({
      ...
      sims_list <- lapply(ns, function(n) {
        try({
          ...
          # re-derive lrnr labels after rbind
          ...
          # ---- choose sieve by CV (or oracle) inside (iter, lrnr, type)
          if (!use_oracle_sieve) {
      ...
      ggsave(fig_file("performancePlot_CATE_xgboost", pos, hard), plot = plt, width = 8, height = 7)
      ...
      ggsave(fig_file("performancePlot_CATE_ranger", pos, hard), plot = plt, width = 8, height = 7)

      # -----------------------------
      # Main-paper combined plot
      # -----------------------------
      dt_tmp <- dt[!(dt$lrnr %in% c("glm", "earth", "gam3", "gam4", "gam5")), ]
      dt_tmp <- dt_tmp[grep("rf", dt$lrnr), ]
      max_depth <- stringr::str_match(dt_tmp$lrnr, "([0-9]+)_xg$")[, 2]
      ...
      ggsave(
        file.path(FIGURES_DIR, paste0("performancePlot_CATE_tree", "pos=", pos, "hard=", hard, "mainpaper.pdf")),
      ...
      # -----------------------------
      for (lrnr in unique(dt_comb$lrnr)) {
        dt_df <- as.data.frame(dt_comb)
        ...
        ggsave(plot_file("performancePlot_CATE_tree_", pos, hard, lrnr),
