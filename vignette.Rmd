...
- Conditional Mean of Treatment: `e(W=w) := E[A | W = w]` (R-only)
...
data.table(pi = hte3_task_custom_sl3$get_nuisance_estimates("pi"),
           ...
           e = hte3_task_custom_sl3$get_nuisance_estimates("e"),
           m = hte3_task_custom_sl3$get_nuisance_estimates("m"))
```
...
Here's a practical illustration of how to train a DR-learner of the cate using generalized additive models (GAMs) as base supervised learning algorithm:
...
hte3_task <- make_hte3_Task_tx(data, modifiers, confounders, treatment, outcome,
                               learner_pi = Lrnr_gam$new(),
                               learner_mu = Lrnr_xgboost$new(max_depth=4),
                               learner_m = Lrnr_xgboost$new(max_depth=4))
```

```{r}
lrnr <- Lrnr_crate_EP$new(Lrnr_ranger$new(family = binomial()), sieve_num_basis = 300, screen_basis_with_lasso = T)
lrnr_trained <- lrnr$train(hte3_task)
plot(lrnr_trained$predict(hte3_task), LRR)

lrnr <- Lrnr_crate_IPW$new(Lrnr_ranger$new(family = binomial()))
lrnr_trained <- lrnr$train(hte3_task)
plot(lrnr_trained$predict(hte3_task), LRR)

lrnr <- Lrnr_crate_T$new(Lrnr_ranger$new(family = poisson()))
lrnr_trained <- lrnr$train(hte3_task)
plot(lrnr_trained$predict(hte3_task), LRR)
```
```{r}
lrnr <- Lrnr_crate_EP$new(Lrnr_gam$new(family = quasibinomial() ))
  
 
lrnr_cv <- cross_validate_crate(lrnr, hte3_task) 
...
ep_lrnr_xg <- cross_validate(make_ep_stack(lrnr_xg_stack, hte3_task), hte3_task)
ep_lrnr_ranger <- cross_validate(make_ep_stack(lrnr_rf, hte3_task), hte3_task)
ep_lrnr_earth <- cross_validate(make_ep_stack(lrnr_earth, hte3_task), hte3_task)
ep_lrnr_gam <- cross_validate(make_ep_stack(lrnr_gam, hte3_task), hte3_task)

#train DR-learners
dr_lrnr_xg <- cross_validate(Lrnr_cate_DR$new(lrnr_xg_stack), hte3_task)
...
#train R-learners
r_lrnr_xg <- cross_validate(Lrnr_cate_R$new(lrnr_xg_stack), hte3_task)
r_lrnr_ranger <- Lrnr_cate_R$new(lrnr_rf)$train(hte3_task)
r_lrnr_gam <- Lrnr_cate_R$new(lrnr_gam)$train(hte3_task)


# train T-learners
t_lrnr_xg <- cross_validate(Lrnr_cate_T$new(lrnr_xg_stack), hte3_task)
...
ep_learner <- Lrnr_cate_EP$new(base_learner = Stack$new(Lrnr_gam$new(), Lrnr_earth$new()), sieve_basisN  = 5, sieve_type = "polytri",
                               treatment_level = 1, control_level = 0, include_ridge_penalty = FALSE)
dr_learner <- Lrnr_cate_DR$new(base_learner = Lrnr_earth$new(), treatment_level = 1, control_level = 0)

cv_ep_learner <- cross_validate(list(ep_learner, dr_learner), hte3_task)
    cv_ep_learner$coefficients
cv_ep_learner$predict(hte3_task)

 

 mu <- hte3_task$get_nuisance_estimates("mu")
cate.hat <- apply(mu, 1 , diff)
...
ep_learner_trained <- ep_learner$train(hte3_task)
plot(cate, ep_learner_trained$predict(hte3_task))


dr_learner_trained <- dr_learner$train(hte3_task)
plot(cate, dr_learner_trained$predict(hte3_task))

sqrt(mean((cate -  cate.hat)^2))
sqrt(mean((cate -  ep_learner_trained$predict(hte3_task))^2))
sqrt(mean((cate -  dr_learner_trained$predict(hte3_task))^2))
...
dr_learner <- Lrnr_cate_DR$new(base_learner = Lrnr_gam$new(), treatment_level = 1, control_level = 0)
r_learner <- Lrnr_cate_R$new(base_learner = Lrnr_gam$new())

dr_learner_trained <- dr_learner$train(hte3_task)
plot(cate, dr_learner_trained$predict(hte3_task))
r_learner_trained <- r_learner$train(hte3_task)
plot(cate, r_learner_trained$predict(hte3_task))


cor(cate, dr_learner_trained$predict(hte3_task))
```


```{r}
# Specify a cate DR-learner with generalized additive models (GAMs) as the supervised learning algorithm
lrnr_cate_dr <- Lrnr_cate_DR$new(base_learner = Lrnr_gam$new(), treatment_level = 1, control_level = 0)
